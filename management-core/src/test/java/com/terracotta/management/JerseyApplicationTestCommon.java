...
public class JerseyApplicationTestCommon {
	private static final String[] PACKAGE_STARTS_WITH_FILTERS = new String[] {
			"java", "javax", "org.apache", "com.sun", "org.codehaus",
			"org.hibernate", "org.glassfish.jersey", "com.fasterxml.jackson" };

	private static final ClassFilter jerseyFilter = new ClassFilter() {
		public boolean accept(Class<?> clazz) {
			return (clazz.isAnnotationPresent(javax.ws.rs.ext.Provider.class) || clazz
					.isAnnotationPresent(javax.ws.rs.Path.class));
		}
	};

	protected Set<Class<?>> annotatedClassesFound() throws IOException,
	...
	private List<String> getClasspathElements() throws IOException {
		...
		if (classpath.contains("surefirebooter")) {
			...
			for (String urlElement : classpath.split(" ")) {
				elements.add(FileUtils.toFile(new URL(urlElement))
						.getAbsolutePath());
			}
		} else {
	...
	private Set<Class<?>> filterAnnotatedClassesFromJar(String cpElement)
			throws IOException {
		Set<Class<?>> classes = new HashSet<Class<?>>();
	...
	private void filterAnnotatedClassesFromRootPath(File file,
			Set<Class<?>> classes, File rootPath) throws ClassNotFoundException {
		if (file.isFile() && file.getAbsolutePath().endsWith(".class")) {
			String replace = file.getAbsolutePath().replace(
					rootPath.getAbsolutePath() + File.separator, "");
			String className = replace.replace(File.separator, ".").substring(
					0, replace.length() - 6);
			try {
				Class<?> clazz = Class.forName(className);
				if (jerseyFilter.accept(clazz)) {
